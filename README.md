...
### Inspectable 3D models

Hover a 3D part to see its supplied name, explanation, and properties; click or tap to pin the details and highlight its bounds. Selecting a part pauses camera rotation. The part selector provides the same inspection without precise pointing; unnamed legacy objects show "Unnamed part" with no invented explanation. Decorations marked `userData.inspectable = false` are excluded. Drag to orbit, scroll to zoom, or use arrow keys while the canvas is focused. Escape clears selection.

The 3D toolbar controls **rotation** and **model animation** separately. Pausing model animation freezes the generated `update(t)` clock; resuming continues without adding the paused interval. **Reset 3D view** (or Home with canvas focus) restores camera orientation and fitted zoom, clears selection, and preserves playback state and model time. Scenes without an animation callback have the animation control disabled. Older generated code with its own timers or clocks cannot be reliably paused by this control; new generation instructions require motion through `update(t)` only.

New 3D generation requests meaningful `object.name` and optional `object.userData.study = {label, description, properties, illustrative}` metadata. Child meshes can inherit a parent group's metadata. Properties are bounded text, numbers, or booleans; conceptual geometry and example values should be marked illustrative. Descriptions remain model-supplied, not independently verified evidence. Existing scenes get controls and selection without regeneration, but only scenes with supplied metadata show explanations. No AI call is made for inspection, camera movement, reset, or playback. Hidden/isolated parts, exploded views, and 3D evidence links are not part of this first version.

Checks: `npm run test:three-inspection` and `npm run test:three-inspection:browser` (built extension and installed Edge required).
...
Bar-chart category labels wrap within measured slots, with separate space reserved for axis captions. Dense bar charts scroll inside the graph pane instead of overlapping or truncating labels. Multi-series line charts use a separate wrapping legend above the plot; long legends scroll vertically within a bounded area, preserving full series names and colors without covering the lines. Charts redraw when the pane changes size, using dimensions rounded down to avoid scrollbar-driven resize oscillation. Line and point plots fit their viewport without canvas scrollbars. These rendering fixes apply to existing saved graphs without regeneration or model calls. Checks: `npm run test:graph-layout` and `npm run test:graph-layout:browser`.

Line charts show small dots at every supplied data point, matching the series color with a contrasting outline. Dots identify stored coordinates, not additional measurements; schematic/generated data remain schematic. Function plots remain continuous curves and scatter plots retain their existing dots.

Hover a point or bar to inspect its series/category and exact x/y values, labeled with the chart axes. Click or tap to pin the details; nearby or overlapping points can be chosen from the pinned panel. Close it with the close button, Escape, another click on the selected point, or an outside click. Focus the chart and use arrow keys or Home/End to inspect points by series/data order; Enter opens details. Tooltips stay inside the graph and clear when the graph, theme, viewport, or scroll position changes. Function values are inspected at sampled coordinates, not arbitrary interpolated positions. Inspection is local and never regenerates the chart or calls an AI provider. Geometry tests: `npm run test:graph-inspection`; browser coverage is included in `npm run test:graph-layout:browser`.

Evidence is stored inside each visualization specification, so revision, undo, and saved-version switching retain the appropriate evidence. Existing results remain usable with **Evidence not checked**; generate a new version to add evidence. Source narratives remain English, but evidence quotations preserve the PDF's original language. Formula and Step by Step use the concept's source page; Source explanations use the retrieval flow below. Graph/Canvas/3D object picking, OCR/image evidence, numerical proof checks, and human review status are not included yet. Inspecting evidence and switching saved versions make no AI requests; generating evidence is part of the normal generation call.

Tests: `npm run test:evidence` and `npm run test:evidence:browser` (built extension and installed Edge required).

### Source paper retrieval

Source generation searches explanatory pages throughout the current PDF and matching papers already in Library. Bibliography pages are excluded from explanatory-page selection. For a bibliography-only reference, the app looks for the actual paper instead of treating its citation as evidence of methods or findings. Context is limited to four complete pages and 40,000 characters; this is lexical retrieval, not exhaustive document analysis.

**Settings > Public paper retrieval** is off by default. Enabling it permits the engine to send a citation title to OpenAlex and download a matching public PDF from arXiv, NeurIPS, PMLR, ACL Anthology, or USENIX. Explicit arXiv IDs skip metadata search. The engine sends no source-page text, credentials, or browser cookies to these services. Copilot's tools remain disabled. Requests use HTTPS, validate every redirect, have deadlines and streamed size limits (20 MB for PDFs), and retain the existing 150-page/text-quality gates. No paywalls, authentication, or organization restrictions are bypassed.

Title, available author/year information, and the PDF's opening text are checked before use. Ambiguous matches, unsupported repositories, scanned PDFs, and unavailable downloads produce an evidence warning. Supply an optional **Paper URL** in the Source controls, or **Import PDF**, then generate again. Matching is conservative and cannot resolve every citation; arbitrary URLs and HTML pages are not downloaded.

Retrieved PDFs and extracted text are cached as separate Library documents, with content-based IDs and their public URLs. Later generations reuse matching library copies without downloading again, even when public retrieval is off. Delete the reference document in Library to remove its cached PDF and extracted text. Existing visualization versions retain their saved quotes/provenance, but their PDF links become unavailable if that reference is deleted. Turning the setting off affects subsequent retrievals; it does not cancel requests already started or delete cached files.

Each result names its actual source document. Cross-document evidence opens the cached source PDF at the cited page in a new tab; it does not highlight the original bibliography. Same-document evidence retains inline PDF highlighting. Source versions preserve their own evidence and previous results are not automatically regenerated.

Tests: `npm run test:source-retrieval` and `npm run test:source-retrieval:browser` (installed Edge and built extension required). The browser suite uses isolated storage and a synthetic provider; it makes no public-paper requests or live model calls.
...
For **Source** concepts, Copilot summarizes text supplied by the application rather than browsing. Background generation and revisions share the Source retrieval flow above. Prompts require quotes to come from supplied text, prohibit invented references, and allow an empty citations list when no source URLs are supplied. Without retrieved external material, the result must disclose that external sources were not verified. Retrieval and excerpt matching are not an independent fact-checker; review important claims against the PDF. Retry an existing Source tag to use retrieval; completed visualizations are not automatically changed.
