...
import { Send, Sparkles, Undo2, MessageSquare, Loader2 } from "lucide-react";
...
export default function RevisionPanel({ docId, tag, onUpdate }: { docId: string; tag: PersistedTagServer; onUpdate: (tag: PersistedTagServer) => void }) {
  ...
  async function submit(action: "generate" | "undo", feedback = "") {
    if (disabled) return;
    setBusy(true); setError("");
    try {
      const response = await fetch(`/api/viz/${encodeURIComponent(docId)}/${encodeURIComponent(tag.id)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(action === "undo" ? { action, revision: tag.revision ?? 0 } : { action, type, feedback, revision: tag.revision ?? 0 }) });
